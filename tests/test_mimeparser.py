...
class TestMIMEParser(unittest.TestCase):
    ...
    def test_parse_mime_quality_domain_errors(self):
        ...
        """
        # Test q below 0
        mime = 'application/vnd.corp.project.endpoint+json;q=-1'
        result = self.mp.parse_mime(mime)
        ...
        # Test q above 1
        mime = 'application/vnd.corp.project.endpoint+json;q=2'
        result = self.mp.parse_mime(mime)
        quality = result[3].get('q')
        msg = "Found a q of: {}, should be 1, mimetype: {}".format(
            quality, mime)
    ...
    def test_parse_mime_quality_non_numeric_errors(self):
        ...
        mime = 'application/vnd.corp.project.endpoint+json;q=no'
        result = self.mp.parse_mime(mime)
        quality = result[3].get('q')
        msg = "Found a q of: {}, should be: 1, mimetype: {}".format(
            quality, mime)
        ...
        realm = 'localhost'
        mime = 'application/vnd.corp.project.endpoint+json;realm='
    ...
    def test_parse_mime_use_quotes_in_params(self):
        ...
        mime = 'application/vnd.corp.project.endpoint+json;q=".9"'
        result = self.mp.parse_mime(mime)
        quality = result[3].get('q')
        ...
        mime = 'application/vnd.corp.project.endpoint+json;charset="utf-8"'
        result = self.mp.parse_mime(mime)
    ...
    def test__fitness_and_quality(self):
        ...
        parsed_header_mtypes = [self.mp.parse_mime(mt) for mt in header_mtypes]
        expected_results = [
            [5, 0, Decimal('0.4')],
            [5, 0, Decimal('0.4')],
            [7, 0, Decimal('0.8')],
        ...
        for idx, available_mtype in enumerate(available_mtypes):
            ...
            result = self.mp._fitness_and_quality(
                available_mtype, parsed_header_mtypes)
            self.assertEqual(result, expected_results[idx], msg.format(
                result, expected_results[idx], available_mtype))

    #@unittest.skip("Temporarily skipped.")
    def test__best_weighted_matches(self):
        """
        Test the flakiness when two mimetypes contend for best match.
        """
        available_mtypes = [
            'application/vnd.corp.project.endpoint+json;ver=1',
            'application/json',
            'text/html',
            ]
        header_mtypes = [
            'application/json',
            'application/vnd.corp.project.endpoint+json;ver=1',
            'application/vnd.corp.project.endpoint+json;ver=2',
            'application/vnd.corp.project.endpoint',
            'application/*',
            '*/json',
            '*/*',
            ]
        ...
        for idx, mtype in enumerate(header_mtypes):
            weighted_matches = self.mp._best_weighted_matches(
                available_mtypes, mtype)
            result_mime = weighted_matches[0][4]
            required_mime = result_mtypes[idx]
            msg = ("result_mime: {}, required_mime: {}, weighted_matches: {}"
                   ).format(result_mime, required_mime, weighted_matches)
            self.assertEqual(result_mime, required_mime, msg)
    ...
    def test_best_match_single_mime_single_mime_list(self):
        ...
        """
        mime = 'application/vnd.corp.project.endpoint+json;q=-1'
        available_mtypes = ['application/vnd.corp.project.endpoint+json']
